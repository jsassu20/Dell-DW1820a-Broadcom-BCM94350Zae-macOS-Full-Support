...
// BE SURE TO ENABLE CLOVER DSDT FIXES "ADD DTGP" AS WELL AS "FIX AIRPORT" IN YOUR config.plist FILE USING CLOVER CONFIGURATOR! 
...
DefinitionBlock("", "SSDT", 2, "ARPT ", "BCM4350", 0)
{
    ...
    {
        Name (_ADR, Zero)
        Device (ARPT) // WAS ORIGINALLY NAMED PXSX AND CHANGED TO ARPT AS IT WOULD BE IN A REAL MAC DEVICE (JUST CHANGE WHATEVER THE LABLE IS TO ARPT)
        {
        ...
        Device (_SB.PCI0.<YourDevice>)   // Identified wireless device through IORegistryExplorer (usually: ARPT)
        {
            Name (_ADR, Zero)
            Method (_DSM, 4, NotSerialized)  // _DSM: Device-Specific Method
            {
                ...
                Return (Package ()
                {
                    ...
                    Buffer (0x0B)
                    {
                        "pci14e4,4331" // PCI id of device supported OOB
                    },  
                    "device_type", 
                    Buffer (0x08)
                    {
